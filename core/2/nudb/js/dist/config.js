...
function isStorageSupported (type) {
  const warn = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false
...
function markField (field) {
  ...
  const isDefault = checkFieldDefault($field, type) // checkboxes uses parent <span> for marking
...
var validators = {
  ...
  validatePositiveNumber: function (isKeyUp) {
    ...
    const result = this.value !== '0' && validators.regExpNumeric.test(this.value)
  ...
  validateNonNegativeNumber: function (isKeyUp) {
    ...
    const result = validators.regExpNumeric.test(this.value)
  ...
  validatePortNumber: function () {
    ...
    const result = validators.regExpNumeric.test(this.value) && this.value !== '0'
  ...
  validateByRegex: function (isKeyUp, regexp) {
    ...
    const parts = regexp.match(validators.regExpPcreExtract)
...
function getFieldValidators (fieldId, onKeyUpOnly) {
  ...
  if (typeof validate[fieldId] !== 'undefined') {
    ...
    for (let i = 0, imax = validate[fieldId].length; i < imax; i++) {
...
function displayErrors (errorList) {
  ...
  for (const fieldId in errorList) {
...
function setDisplayError () {
  const elements = $('.optbox input[id], .optbox select[id], .optbox textarea[id]') // run all field validators
...
function validateFieldset (fieldset, isKeyUp, errors) {
  ...
  if ($fieldset.length && typeof validators.fieldset[$fieldset.attr('id')] !== 'undefined') {
    const fieldsetErrors = validators.fieldset[$fieldset.attr('id')].apply($fieldset[0], [isKeyUp])

    for (const fieldId in fieldsetErrors) {
...
function setupValidation () {
  ...
  $elements.each(function () {
    ...
    const tagName = $el.attr('tagName') // text fields can be validated after each change
...
function updatePrefsDate () {
  ...
  const msg = Messages.strSavedOn.replace('@DATE@', Functions.formatDateTime(d))
