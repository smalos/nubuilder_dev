...
var Console = {
  ...
  collapse: function () {
    ...
    const pmaConsoleHeight = Math.max(92, Console.config.Height)
  ...
  show: function (inputFocus) {
    ...
    let pmaConsoleHeight = Math.max(92, Console.config.Height) // eslint-disable-next-line compat/compat
...
var ConsoleMessages = {
  ...
  append: function (msgString, msgType) {
    ...
    const now = new Date()
    const $newMessage = $('<div class="message ' + (Console.config.AlwaysExpand ? 'expanded' : 'collapsed') + '" msgid="' + msgId + '"><div class="action_content"></div></div>')
...
var ConsoleDebug = {
  ...
  initialize: function () {
    ...
    const orderBy = Console.config.OrderBy
    const order = Console.config.Order
  ...
  formatQueryOrGroup: function (queryInfo, totalTime) {
    ...
    const $query = $('<div class="message collapsed hide_trace">').append($('#debug_console').find('.templates .debug_query').clone()).append($('<div class="query">').text(queryText)).data('queryInfo', queryInfo).data('totalTime', totalTime)
  ...
  showLog: function (debugInfo, url) {
    ...
    let uniqueQueries = {}
    const totalExec = allQueries.length // Calculate total time and make unique query array
    ...
    function sortByTime (a, b) {
      const order = Console.config.Order === 'asc' ? 1 : -1
    ...
    function sortByCount (a, b) {
      const order = Console.config.Oorder === 'asc' ? 1 : -1
