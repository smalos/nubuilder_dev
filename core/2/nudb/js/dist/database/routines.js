...
const DatabaseRoutines = {
  ...
  validate: function () {
     ...
     */
    let $elm = null // Common validation. At the very least the name
  ...
  exportDialog: function ($this) {
    ...
    if ($this.attr('id') === 'bulkActionExportButton') {
      ...
      let returnCount = 0 // No routine is exportable (due to privilege issues)
          ...
          if (that.validate()) {
            ...
            $.post(url, data, function (data) {
              if (data.success === true) {
                ...
                const tableId = '#' + data.tableType + 'Table' // If we are in 'edit' mode, we must
  ...
  dropDialog: function ($this) {
    ...
    const question = $('<div></div>').text($currRow.children('td').children('.drop_sql').html()) // We ask for confirmation first here, before submitting the ajax request
    ...
    $this.confirm(question, $this.attr('href'), function (url) {
      ...
      const params = Functions.getJsConfirmCommonParam(this, $this.getPostData())
      $.post(url, params, function (data) {
        if (data.success === true) {
           ...
           */
          const $table = $currRow.parent().parent() // Check how many rows will be left after we remove
  ...
  dropMultipleDialog: function ($this) {
    ...
    $this.confirm(Messages.strDropRTEitems, '', function () {
       ...
       *          the AJAX message shown to the user
       */
      const $msg = Functions.ajaxShowMessage(Messages.strProcessingRequest) // drop anchors of all selected rows
  ...
  postDialogShow: function (data) {
    ...
    const that = this // Make adjustments in the dialog to make it AJAX compatible
  ...
  setOptionsForParameter: function ($type, $len, $text, $num) {
    ...
    const $noLen = $len.parent().parent().find('.no_len') // Process for parameter options
  ...
  executeDialog: function ($this) {
    ...
    let $msg = Functions.ajaxShowMessage()
    const params = Functions.getJsConfirmCommonParam($this[0], $this.getPostData())
