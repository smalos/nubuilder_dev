...
AJAX.registerOnload('database/search.js', function () {
  ...
  const icon = Functions.getImage('s_tbl', '', {
    id: 'table-image'
  }).toString()
  ...
  $(document).on('click', 'a.browse_results', function (e) {
    ...
    const $msg = Functions.ajaxShowMessage(Messages.strBrowsing, false)
    $('#sqlqueryform').hide()
    $('#togglequerybox').hide()
    ...
    const url = $(this).attr('href') + '#searchresults'
  ...
  $(document).on('click', 'a.delete_results', function (e) {
    ...
    const msg = Functions.sprintf(Messages.strConfirmDeleteResults, $(this).data('table-name'))

    if (confirm(msg)) {
      const $msg = Functions.ajaxShowMessage(Messages.strDeleting, false)
  ...
  $(document).on('submit', '#db_search_form.ajax', function (event) {
    ...
    const $msgbox = Functions.ajaxShowMessage(Messages.strSearching, false) // jQuery object to reuse
