...
DatabaseStructure.adjustTotals = function () {
  const byteUnits = [Messages.strB, Messages.strKiB, Messages.strMiB, Messages.strGiB, Messages.strTiB, Messages.strPiB, Messages.strEiB]
  /**
   * @var $allTr jQuery object that references all the rows in the list of tables
   */

  const $allTr = $('#tablesForm').find('table.data tbody').first().find('tr') // New summary values for the table
  ...
  $allTr.each(function () {
    const $this = $(this)
    let i
    let tmpVal // Get the number of rows for this SQL table

    let strRows = $this.find('.tbl_rows').text() // If the value is approximated
    ...
    const strOverheadUnit = $this.find('.tbl_overhead span.unit').text().trim() // Given a value and a unit, such as 100 and KiB, for the table size
...
AJAX.registerOnload('database/structure.js', function () {
  ...
  $(document).on('click', 'a.truncate_table_anchor.ajax', function (event) {
    ...
    const $thisAnchor = $(this) // extract current table name and build the question string
    ...
    const question = Messages.strTruncateTableStrongWarning + ' ' + Functions.sprintf(Messages.strDoYouReally, 'TRUNCATE `' + Functions.escapeHtml(currTableName) + '`') + Functions.getForeignKeyCheckboxLoader()
    $thisAnchor.confirm(question, $thisAnchor.attr('href'), function (url) {
      Functions.ajaxShowMessage(Messages.strProcessingRequest)
      const params = Functions.getJsConfirmCommonParam(this, $thisAnchor.getPostData())
  ...
  $(document).on('click', '#real_end_input', function (event) {
    ...
    const question = Messages.strOperationTakesLongTime
