...
const mainDirection = $('html').attr('dir') === 'rtl' ? 'right' : 'left' // Will be used to multiply the offsetLeft by -1 if the direction is rtl.
...
let onRelation = 0
let onGrid = 0
let onDisplayField = 0 // relation_style: 0 - angular 1 - direct
...
const isIe = document.all && !window.opera
...
DesignerMove.mouseMove = function (e) {
  ...
  const newDx = isIe ? e.clientX + document.body.scrollLeft : e.pageX // eslint-disable-next-line compat/compat

  const newDy = isIe ? e.clientY + document.body.scrollTop : e.pageY
    ...
    for (key in contr[K]) {
      ...
      for (key2 in contr[K][key]) {
        ...
        for (key3 in contr[K][key][key2]) {
          ...
          const x2Left = document.getElementById(contr[K][key][key2][key3][0]).offsetLeft
          const x2Right = x2Left + document.getElementById(contr[K][key][key2][key3][0]).offsetWidth
          ...
          if (tabHideButton.innerHTML === 'v') {
            const toColumn = document.getElementById(contr[K][key][key2][key3][0] + '.' + contr[K][key][key2][key3][1])
          ...
          const y2 = document.getElementById(contr[K][key][key2][key3][0]).offsetTop + rowOffsetTop + heightField
...
DesignerMove.addTableToTablesList = function (index, tableDom) {
  ...
  const checkboxStatus = tableIsChecked === 'checked' ? Messages.strHide : Messages.strShow
  const $newTableLine = $('<tr>' + '    <td title="' + Messages.strStructure + '"' + '        width="1px"' + '        class="L_butt2_1">' + '        <img alt=""' + '            db="' + dbEncoded + '"' + '            table_name="' + tableEncoded + '"' + '            class="scroll_tab_struct"' + '            src="' + themeImagePath + 'designer/exec.png"/>' + '    </td>' + '    <td width="1px">' + '        <input class="scroll_tab_checkbox"' + '            title="' + checkboxStatus + '"' + '            id="check_vis_' + dbEncoded + '.' + tableEncoded + '"' + '            style="margin:0;"' + '            type="checkbox"' + '            value="' + dbEncoded + '.' + tableEncoded + '"' + tableIsChecked + '            />' + '    </td>' + '    <td class="designer_Tabs"' + '        designer_url_table_name="' + dbEncoded + '.' + tableEncoded + '">' + $('<div/>').text(db + '.' + table).html() + '</td>' + '</tr>')
...
DesignerMove.addOtherDbTables = function () {
  ...
  const $form = $('<form action="" class="ajax"></form>').append($selectDb).append($selectTable)
  const modal = DesignerMove.displayModal($form, Messages.strAddTables, '#designerGoModal')
  $('#designerModalGoButton').on('click', function () {
    const db = $('#add_table_from').val()
    const table = $('#add_table').val() // Check if table already imported or not.
  ...
  if (designerTablesEnabled || forceString) {
    ...
    for (key in jTabs) {
      if (document.getElementById('check_vis_' + key).checked) {
        ...
        const tbCoords = new DesignerObjects.TableCoordinate($(document.getElementById(key)).attr('db_url'), $(document.getElementById(key)).attr('table_name_url'), -1, x, y)
  ...
  DesignerMove.promptToSaveCurrentPage(function () {
    ...
    $.post('index.php?route=/database/designer', {
      ...
      if (data.success === false) {
        ...
        const modal = DesignerMove.displayModal(data.message, Messages.strOpenPage, '#designerGoModal')
        $('#designerModalGoButton').on('click', function () {
...
DesignerMove.deletePages = function () {
  ...
  $.post('index.php?route=/database/designer', {
    ...
    if (data.success === false) {
      ...
      const modal = DesignerMove.displayModal(data.message, Messages.strDeletePage, '#designerGoModal')
      $('#designerModalGoButton').on('click', function () {
        const $form = $('#edit_delete_pages')
        const selected = $form.find('select[name="selected_page"]').val()
        ...
        const $messageBox = Functions.ajaxShowMessage(Messages.strProcessingRequest)
        const deletingCurrentPage = parseInt(selected) === selectedPage
        Functions.prepareForAjaxRequest($form)
...
DesignerMove.saveAs = function () {
  ...
  $.post('index.php?route=/database/designer', {
    ...
    if (data.success === false) {
      ...
      const modal = DesignerMove.displayModal(data.message, Messages.strSavePageAs, '#designerGoModal')
      $('#designerModalGoButton').on('click', function () {
...
DesignerMove.promptToSaveCurrentPage = function (callback) {
  if (change === 1 || selectedPage === -1) {
    const modal = DesignerMove.displayModal('<div>' + Messages.strLeavingPage + '</div>', Messages.strSavePage, '#designerPromptModal')
...
DesignerMove.exportPages = function () {
  ...
  $.post('index.php?route=/database/designer', {
    ...
    if (data.success === false) {
      ...
      const modal = DesignerMove.displayModal($form, Messages.strExportRelationalSchema, '#designerGoModal')
      $('#designerModalGoButton').on('click', function () {
...
DesignerMove.clickField = function (db, T, f, pk) {
  ...
  if (onDisplayField) {
    ...
    const oldTabField = document.getElementById('id_tr_' + T + '.' + displayField[T]) // if is display field
    ...
    if (displayField[T] === f) {
      ...
      displayField[T] = f
      const tabField = document.getElementById('id_tr_' + T + '.' + displayField[T])
...
DesignerMove.canvasClick = function (id, event) {
  ...
  let key
  let key2
  let key3 // eslint-disable-next-line compat/compat

  let localX = isIe ? event.clientX + document.body.scrollLeft : event.pageX // eslint-disable-next-line compat/compat

  let localY = isIe ? event.clientY + document.body.scrollTop : event.pageY
    ...
    for (key in contr[K]) {
      for (key2 in contr[K][key]) {
        for (key3 in contr[K][key][key2]) {
          ...
          const x1Right = x1Left + document.getElementById(key2).offsetWidth
          const x2Left = document.getElementById(contr[K][key][key2][key3][0]).offsetLeft // +document.getElementById(contr[K][key2][key3][0]+"."+contr[K][key2][key3][1]).offsetLeft
          ...
          const y2 = document.getElementById(contr[K][key][key2][key3][0]).offsetTop + document.getElementById(contr[K][key][key2][key3][0] + '.' + contr[K][key][key2][key3][1]).offsetTop + heightField
...
DesignerMove.sideMenuRight = function (idThis) {
  ...
  const resizeIcon = $('#layer_menu_sizer > img').toggleClass('float-start').toggleClass('float-end')
...
DesignerMove.getColorByTarget = function (target) {
  ...
  for (const targetColor in TargetColors) {
...
DesignerMove.selectAll = function (tableName, dbName, idSelectAll) {
  ...
  const checkboxAll = $('#container-form input[id_check_all=\'' + idSelectAll + '\']:checkbox')
...
DesignerMove.addObject = function (dbName, tableName, colName, dbTableNameUrl) {
  ...
  let sum = 0
  const init = historyArray.length
  ...
  if (document.getElementById('new_name').value !== '') {
    const renameObj = new DesignerHistory.Rename(document.getElementById('new_name').value) // make Rename object
  ...
  if (document.getElementById('operator').value !== '---') {
    const aggregateObj = new DesignerHistory.Aggregate(document.getElementById('operator').value)
  ...
  if (document.getElementById('orderby').value !== '---') {
    const orderByObj = new DesignerHistory.OrderBy(document.getElementById('orderby').value)
