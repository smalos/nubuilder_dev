...
AJAX.registerOnload('export.js', function () {
  ...
  $('input[name="createTemplate"]').on('click', function (e) {
    e.preventDefault()
    const name = $('input[name="templateName"]').val()
...
Export.toggleStructureDataOpts = function () {
  ...
  const show = $('input[type=\'radio\'][name=\'' + radioFormName + '\']:checked').val() // Show the #rows if 'show' is not structure
...
AJAX.registerOnload('export.js', function () {
  ...
  $('#plugins').on('change', function () {
    const activePlugin = $('#plugins').find('option:selected').val()
    const forceFile = $('#force_file_' + activePlugin).val()
  ...
  if ($('input[name=\'export_type\']').val() === 'database') {
    ...
    const dummyCreateTable = $('#checkbox_sql_create_table').clone().removeAttr('id').attr('type', 'hidden')
...
AJAX.registerOnload('export.js', function () {
  ...
  const setSelectOptions = function (doCheck) {
  ...
  $('#buttonGo').on('click', function () {
    const timeLimit = parseInt($(this).attr('data-exec-time-limit')) // If the time limit set is zero,
