...
Indexes.removeColumnFromIndex = function (colIndex) {
  ...
  let previousIndex = $('select[name="field_key[' + colIndex + ']"]').attr('data-index')

  if (previousIndex.length) {
    previousIndex = previousIndex.split(',')
    const sourceArray = Indexes.getIndexArray(previousIndex[0])

    if (sourceArray === null) {
      return
    } // Remove column from index array.

    const sourceLength = sourceArray[previousIndex[1]].columns.length
...
Indexes.addColumnToIndex = function (sourceArray, arrayIndex, indexChoice, colIndex) {
  ...
  const indexName = $('input[name="index[Key_name]"]').val()
...
Indexes.getCompositeIndexList = function (sourceArray, colIndex) {
  ...
  const $compositeIndexList = $('<ul id="composite_index_list">' + '<div>' + Messages.strCompositeWith + '</div>' + '</ul>') // Add each column to list available for composite index.
...
Indexes.showAddIndexDialog = function (sourceArray, arrayIndex, targetColumns, colIndex, index, showDialog) {
  const showDialogLocal = typeof showDialog !== 'undefined' ? showDialog : true // Prepare post-data.
  ...
  for (let i = 0; i < targetColumns.length; i++) {
    const columnName = $('input[name="field_name[' + targetColumns[i] + ']"]').val()
    const columnType = $('select[name="field_type[' + targetColumns[i] + ']"]').val().toLowerCase()
...
Indexes.indexTypeSelectionDialog = function (sourceArray, indexChoice, colIndex) {
  const $singleColumnRadio = $('<input type="radio" id="single_column" name="index_choice"' + ' checked="checked">' + '<label for="single_column">' + Messages.strCreateSingleColumnIndex + '</label>')
  const $compositeIndexRadio = $('<input type="radio" id="composite_index"' + ' name="index_choice">' + '<label for="composite_index">' + Messages.strCreateCompositeIndex + '</label>')
  const $dialogContent = $('<fieldset class="pma-fieldset" id="advance_index_creator"></fieldset>')
  ...
  const buttonOptions = {} // 'OK' operation.
  ...
  buttonOptions[Messages.strGo] = function () {
    ...
    if ($('#composite_index').is(':checked')) {
      ...
      const arrayIndex = $('input[name="composite_with"]:checked').val()
...
AJAX.registerOnload('indexes.js', function () {
  ...
  $(document).on('click', 'a.drop_primary_key_index_anchor.ajax', function (event) {
    ...
    for (let i = 1, $lastRow = $currRow.next(); i < rows; i++, $lastRow = $lastRow.next()) {
    ...
    Functions.confirmPreviewSql(question, $anchor.attr('href'), function (url) {
      const $msg = Functions.ajaxShowMessage(Messages.strDroppingPrimaryKeyIndex, false)
  ...
  $(document).on('click', '#table_index tbody tr td.edit_index.ajax, #index_div .add_index.ajax', function (event) {
    ...
    if ($(this).find('a').length === 0) {
      // Add index
      const valid = Functions.checkFormElementInRange($(this).closest('form')[0], 'added_fields', 'Column count has to be larger than zero.')
  ...
  $(document).on('click', '#table_index tbody tr td.rename_index.ajax', function (event) {
    ...
    const title = Messages.strRenameIndex
    url += CommonParams.get('arg_separator') + 'ajax_request=true'
  ...
  $('body').on('change', 'select[name*="field_key"]', function (e, showDialog) {
    const showDialogLocal = typeof showDialog !== 'undefined' ? showDialog : true // Index of column on Table edit and create page.

    let colIndex = /\d+/.exec($(this).attr('name'))
    colIndex = colIndex[0] // Choice of selected index.

    let indexChoice = /[a-z]+/.exec($(this).val())
  ...
  $(document).on('click', '.show_index_dialog', function (e) {
    ...
    const previousIndex = $(this).prev('select').attr('data-index').split(',')
