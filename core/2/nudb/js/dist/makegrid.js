...
const makeGrid = function (t, enableResize, enableReorder, enableVisib, enableGridEdit) {
  ...
  var g = {
      ...
      if (g.colRsz) {
        ...
        if (hoveredCol) {
          ...
          if (newn !== g.colReorder.n) {
            ...
            const newleft = newn < g.colReorder.n ? colPos.left : colPos.left + $(hoveredCol).outerWidth()
            $(g.cPointer).css({
              left: newleft,
              visibility: 'visible'
            })
    ...
    dragEnd: function (e) {
      if (g.colRsz) {
        ...
        const n = g.colRsz.n // do the resizing
    ...
    refreshRestoreButton: function () {
      ...
      const isOneColumn = g.visibleHeadersCount === 1 // enable or disable restore button
    ...
    showEditCell: function (cell) {
      if ($(cell).is('.grid_edit') && !g.colRsz && !g.colReorder) {
        if (!g.isCellEditActive) {
          ...
          if ($cell.attr('data-type') === 'string' || $cell.attr('data-type') === 'blob' || $cell.attr('data-type') === 'json') {
          ...
          let value = Functions.getCellValue(cell)
      ...
      if (data) {
        if (g.currentEditCell) {
          ...
          if (isNull) {
            ...
            let value = data.isNeedToRecheck ? data.truncatableFieldValue : $thisField.data('value') // Truncates the text.
            ...
            let newHtml = Functions.escapeHtml(value)
            newHtml = newHtml.replace(/\n/g, '<br>\n')
            const decimals = parseInt($thisField.attr('data-decimals')) // remove decimal places if column type not supported
        ...
        if (data.transformations !== undefined) {
          $.each(data.transformations, function (cellIndex, value) {
            const $thisField = $(g.t).find('.to_be_saved').eq(cellIndex)
    ...
    showEditArea: function () {
      if (!g.isCellEditActive) {
        ...
        const fieldName = Sql.getFieldName($(t), $td)
        /**
        ...
        const currValue = $td.find('span').text() // empty all edit area, then rebuild it based on $td classes

        $editArea.empty() // remember this instead of testing more than once

        const isNull = $td.is('.null') // add goto link, if this cell contains a link
        ...
        if ($td.is(':not(.not_null)')) {
          ...
          const $checkbox = $editArea.find('.null_div input') // check if current <td> is NULL
        ...
        if ($td.is('.relation')) {
          ...
          if ($td.is('.to_be_saved')) {
            ...
            const sqlQuery = 'SELECT `' + fieldName + '` FROM `' + g.table + '` WHERE ' + whereClause // Make the Ajax call and get the data, wrap it and insert it
          ...
          const $inputField = $(g.cEdit).find('.edit_box') // remember current datetime value in $input_field, if it is not null

          let datetimeValue = !isNull ? $inputField.val() : ''
          let showMillisec = false
          let showMicrosec = false
          let timeFormat = 'HH:mm:ss' // check for decimal places of seconds
    ...
    postEditedCell: function (options) {
      ...
      const multiEditFieldsNull = [] // alert user if edited table is not unique
      ...
      $(g.t).find('td.to_be_saved').parents('tr').each(function () {
        ...
        const conditionArray = JSON.parse($tr.find('.condition_array').val())
        /**
        ...
        const fieldsType = []
        const fields = []
        const fieldsNull = [] // loop each edited cell in a row
        ...
        $tr.find('.to_be_saved').each(function () {
          ...
          const fieldName = Sql.getFieldName($(g.t), $thisField)
          /**
          ...
          if (whereClause.indexOf(Sql.urlEncode(fieldName)) > -1) {
            ...
            for (const field in conditionArray) {
        ...
        success: function (data) {
          ...
          if (typeof data !== 'undefined' && data.success === true) {
            ...
            if (typeof data.sql_query !== 'undefined') {
              ...
              const tools = $resultQuery.find('.tools').wrap('<p>').parent().html() // sqlOuter and tools will not be present if 'Show SQL queries' configuration is off

              if (typeof sqlOuter !== 'undefined' && typeof tools !== 'undefined') {
                $(g.o).find('.result_query').not($(g.o).find('.result_query').last()).remove()
                const $existingQuery = $(g.o).find('.result_query') // If two query box exists update query in second else add a second box
    ...
    saveEditedCell: function () {
       ...
       */
      const $thisField = $(g.currentEditCell)
      let $testElement = '' // to test the presence of a element
      ...
      if ($(g.cEdit).find('.edit_area').is('.edit_area_loading')) {
        ...
        if ($thisField.is('.bit')) {
          ...
          if ($(g.cEdit).find('.edit_box').val().match(/^(0x)?[a-f0-9]*$/i) !== null) {
            ...
            const hexError = '<div class="alert alert-danger" role="alert">' + Messages.strEnterValidHex + '</div>'
    ...
    saveOrPostEditedCell: function (options) {
      const saved = g.saveEditedCell() // Check if $cfg['SaveCellsAtOnce'] is false
    ...
    initColResize: function () {
      ...
      const $firstRowCols = $(g.t).find('tr').first().find('th.draggable') // create column borders
    ...
    initColReorder: function () {
      ...
      const $firstRowCols = $(g.t).find('tr').first().find('th.draggable') // initialize column order
      ...
      $(g.t).find('th.draggable').on('mousedown', function (e) {
        ...
        e.preventDefault()
        const res = Functions.copyToClipboard($(this).data('column'))
    ...
    initColVisib: function () {
      ...
      let i // initialize column visibility
      ...
      if ($firstRowCols.length > 1) {
        const $colVisibTh = $(g.t).find('th:not(.draggable)').slice(0, 1)
    ...
    initGridEdit: function () {
      ...
      $(g.t).find('td.data.click2').on('click', function (e) {
        const $cell = $(this) // In the case of relational link, We want single click on the link
  ...
  const $firstRowCols = $(t).find('tr').first().find('th.draggable') // initialize visible headers count
...
(function ($) {
  $.fn.noSelect = function (p) {
    ...
    const isMsie = navigator.userAgent.indexOf('MSIE') > -1 || !!window.navigator.userAgent.match(/Trident.*rv:11\./)
