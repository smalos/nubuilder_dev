...
Navigation.treeStateUpdate = function () {
  ...
  if (isStorageSupported('sessionStorage')) {
    const storage = window.sessionStorage // try catch necessary here to detect whether
...
Navigation.filterStateRestore = function () {
  if (isStorageSupported('sessionStorage') && typeof window.sessionStorage.navTreeSearchFilters !== 'undefined') {
    ...
    const $tableFilters = $('#pma_navigation_tree li.database').children('div.list_container').find('li.fast_filter input.searchClause') // restore table filters

    $tableFilters.each(function () {
      $obj = $(this).closest('div.list_container') // aPath associated with this filter

      const filterName = $(this).siblings('input[name=aPath]').val() // if this table's filter has a state stored in storage
...
$(function () {
  ...
  $(document).on('click', '#pma_navigation_tree li.new_index a.ajax', function (event) {
    event.preventDefault()
    const url = $(this).attr('href').substr($(this).attr('href').indexOf('?') + 1) + CommonParams.get('arg_separator') + 'ajax_request=true'
    const title = Messages.strAddIndex
  ...
  $(document).on('click', 'li.index a.ajax', function (event) {
    event.preventDefault()
    const url = $(this).attr('href').substr($(this).attr('href').indexOf('?') + 1) + CommonParams.get('arg_separator') + 'ajax_request=true'
    const title = Messages.strEditIndex
    Functions.indexEditorDialog(url, title)
  })
  /** New view */
  ...
  $(document).on('click', 'a.unhideNavItem.ajax', function (event) {
    ...
    const $hiddenTableCount = $tr.parents('tbody').children().length
    const $hideDialogBox = $tr.closest('div.ui-dialog')
    const $msg = Functions.ajaxShowMessage()
    const argSep = CommonParams.get('arg_separator')
    let params = $(this).getPostData()
    params += argSep + 'ajax_request=true' + argSep + 'server=' + CommonParams.get('server')
  ...
  $(document).on('click', '.favorite_table_anchor', function (event) {
    ...
    const hasLocalStorage = isStorageSupported('localStorage') && typeof window.localStorage.favoriteTables !== 'undefined'
    $.ajax({
      url: $self.attr('href'),
      cache: false,
      type: 'POST',
  ...
  if (isStorageSupported('sessionStorage')) {
    const storage = window.sessionStorage // remove tree from storage if Navi_panel config form is submitted
...
Navigation.expandTreeNode = function ($expandElem, callback) {
  ...
  if ($expandElem.hasClass('loaded')) {
    ...
    const $throbber = $('#pma_navigation').find('.throbber').first().clone().css({
      visibility: 'visible',
      display: 'block'
    }).on('click', false)
    $icon.hide()
...
Navigation.showCurrent = function () {
  ...
  function fullExpand (table, $dbItem) {
    const $expander = $dbItem.children('div').first().children('a.expander') // if not loaded or loaded but collapsed
  ...
  function findLoadedItem ($container, name, clazz, doSelect) {
    ...
    $container.children('ul').children('li').each(function () {
      const $li = $(this) // this is a navigation group, recurse
  ...
  function loadAndHighlightTableOrView ($dbItem, itemName) {
    const $container = $dbItem.children('div.list_container')
    let $expander
    let $whichItem = isItemInContainer($container, itemName, 'li.table, li.view') // If item already there in some container
    ...
    if ($whichItem) {
      ...
      const $subContainers = $dbItem.find('.subContainer') // If there are subContainers i.e. tableContainer or viewContainer
