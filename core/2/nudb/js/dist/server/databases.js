...
AJAX.registerOnload('server/databases.js', function () {
  ...
  $(document).on('submit', '#dbStatsForm', function (event) {
    ...
    const selectedDbs = [] // loop over all checked checkboxes, except the .checkall_box checkbox
    ...
    const question = Messages.strDropDatabaseStrongWarning + ' ' + Functions.sprintf(Messages.strDoYouReally, selectedDbs.join('<br>'))
  ...
  $(document).on('submit', '#create_database_form.ajax', function (event) {
    event.preventDefault()
    const $form = $(this) // TODO Remove this section when all browsers support HTML5 "required" property
