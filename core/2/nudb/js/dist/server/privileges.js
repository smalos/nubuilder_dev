...
function exportPrivilegesModalHandler (data, msgbox) {
  if (typeof data !== 'undefined' && data.success === true) {
    const modal = $('#exportPrivilegesModal') // Remove any previous privilege modal data, if any
...
AJAX.registerOnload('server/privileges.js', function () {
  ...
  $(document).on('click', '#deleteUserCard .btn.ajax', function (event) {
    ...
    $thisButton.confirm(Messages.strDropUserWarning, $form.attr('action'), function (url) {
      ...
      if ($dropUsersDbCheckbox.is(':checked')) {
        const isConfirmed = confirm(Messages.strDropDatabaseStrongWarning + '\n' + Functions.sprintf(Messages.strDoYouReally, 'DROP DATABASE'))
      ...
      $.post(url, $form.serialize() + argsep + 'delete=' + $thisButton.val() + argsep + 'ajax_request=true', function (data) {
        if (typeof data !== 'undefined' && data.success === true) {
          ...
          $form.find('input:checkbox:checked').parents('tr').slideUp('medium', function () {
            const thisUserInitial = $(this).find('input:checkbox').val().charAt(0).toUpperCase()
  ...
  const addOrUpdateSubmenu = function () {
    let $subNav = $('.nav-pills')
    const $editUserDialog = $('#edit_user_dialog')
    let submenuLabel
    let submenuLink
    let linkNumber // if submenu exists yet, remove it first
  ...
  const tableSelectAll = function (e) {
