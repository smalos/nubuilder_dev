...
AJAX.registerOnload('server/status/monitor.js', function () {
  ...
  $('body').on('click', function (event) {
    $('div.openedPopup').each(function () {
      const $cnt = $(this)
      const pos = $cnt.offset() // Hide if the mouseclick is outside the popupcontent
...
AJAX.registerOnload('server/status/monitor.js', function () {
  ...
  let oldChartData = null // Holds about to be created chart

  let newChart = null
  let chartSpacing // Whenever the monitor object (runtime.charts) or the settings object
  ...
  const monitorProtocolVersion = '1.0' // Runtime parameter of the monitor, is being fully set in initGrid()
  ...
  let selectionTimeDiff = []
  let selectionStartX
  let selectionStartY
  let drawTimeSpan = false
  ...
  $('a[href="#importMonitorConfig"]').on('click', function (event) {
    ...
    dlgBtns[Messages.strImport] = function () {
      const input = $('#emptyDialog').find('#import_file')[0]
      ...
      reader.onload = function (e) {
        ...
        let json = null // Try loading config
  ...
  $('a[href="#monitorInstructionsDialog"]').on('click', function (event) {
    event.preventDefault()
    const $dialog = $('#monitorInstructionsDialog')
    const dlgBtns = {}
    ...
    const loadLogVars = function (getvars) {
      const vars = {
        ajax_request: true,
      ...
      $.post('index.php?route=/server/status/monitor/log-vars', vars, function (data) {
        ...
        let icon = Functions.getImage('s_success')
        let msg = ''
        let str = ''
  ...
  $('a[href="#submitAddSeries"]').on('click', function (event) {
    ...
    let str = serie.display === 'differential' ? ', ' + Messages.strDifferential : ''
  ...
  function rebuildGrid () {
    ...
    if (runtime.charts) {
      ...
      $.each(runtime.charts, function (key, chartObj) {
        for (let i = 0, l = chartObj.nodes.length; i < l; i++) {
          oldData[chartObj.nodes[i].dataPoint] = []

          for (let j = 0, ll = chartObj.chart.series[i].data.length; j < ll; j++) {
  ...
  function addChart (chartObj, initialize) {
    ...
    const tempTooltipContentEditor = function (str, seriesIndex, pointIndex, plot) {
      let j // TODO: move style to theme CSS

      let tooltipHtml = '<div id="tooltip_editor">' // x value i.e. time
      ...
      for (j in plot.series) {
        ...
        const seriesColor = plot.series[j].color // format y value
    ...
    const $legend = $('<div></div>').css('padding', '0.5em')
    ...
    $('#gridchart' + runtime.chartAI).on('jqplotMouseDown', function (ev, gridpos, datapos) {
      ...
      const selectionBox = $('<div id="selection_box" >') // eslint-disable-next-line compat/compat
    ...
    $('#gridchart' + runtime.chartAI).on('jqplotMouseUp', function (ev, gridpos, datapos) {
      ...
      const min = new Date(Math.ceil(selectionTimeDiff[0]))
      const max = new Date(Math.ceil(selectionTimeDiff[1]))
  ...
  function loadLog (type, min, max) {
    const dateStart = Date.parse($('#logAnalyseDialog').find('input[name="dateStart"]').datepicker('getDate')) || min
    const dateEnd = Date.parse($('#logAnalyseDialog').find('input[name="dateEnd"]').datepicker('getDate')) || max
    loadLogStatistics({
      src: type,
      start: dateStart,
      end: dateEnd,
  ...
  function refreshChartGrid () {
    ...
    runtime.refreshRequest = $.post('index.php?route=/server/status/monitor/chart', {
      ...
      $.each(runtime.charts, function (orderKey, elem) {
        const key = elem.chartID // If newly added chart, we have no data for it yet
  ...
  function buildRequiredDataList () {
    ...
    $.each(runtime.charts, function (key, chart) {
      ...
      for (let i = 0, l = chart.nodes.length; i < l; i++) {
  ...
  function loadLogStatistics (opts) {
    ...
    function filterQueries (varFilterChange) {
      ...
      const columnSums = {} // For the slow log we have to count many columns (query_time, lock_time, rows_examined, rows_sent, etc.)
      ...
      $('#logTable').find('table tbody tr').children('td').eq(runtime.logDataCols.length - 2).each(function () {
        const $t = $(this) // If query is a SELECT and user enabled or disabled to group
  ...
  function buildLogTable (data, groupInserts) {
    ...
    const cols = []
    const $table = $('<table class="table table-light table-striped table-hover align-middle sortable"></table>')
    let $tBody
    let $tRow
    let $tCell
    $('#logTable').html($table)
    ...
    for (let i = 0, l = rows.length; i < l; i++) {
      ...
      for (let j = 0, ll = cols.length; j < ll; j++) {
  ...
  function loadQueryAnalysis (rowData) {
    ...
    $.post('index.php?route=/server/status/monitor/query', {
      ...
      let totalTime = 0 // Float sux, I'll use table :(
      ...
      let explain = '<b>' + Messages.strExplainOutput + '</b> ' + $('#explain_docu').html()
      ...
      const tempExplain = function (key, value) {
        let newValue = value === null ? 'null' : Functions.escapeHtml(value)
      ...
      if (data.profiling) {
        const chartData = []
        let numberTable = '<table class="table table-sm table-light table-striped table-hover w-auto queryNums"><thead><tr><th>' + Messages.strStatus + '</th><th>' + Messages.strTime + '</th></tr></thead><tbody>'
        let duration
        let otherTime = 0
