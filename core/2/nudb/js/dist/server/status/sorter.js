...
$(function () {
  $.tablesorter.addParser({
    ...
    format: function (s) {
      const num = jQuery.tablesorter.formatFloat(s.replace(Messages.strThousandsSeparator, '').replace(Messages.strDecimalSeparator, '.'))
      let factor = 1
  ...
  $.tablesorter.addParser({
    ...
    format: function (s, table, html) {
      const res = html.innerHTML.match(/<span(\s*style="display:none;"\s*)?\s*class="original">(.*)?<\/span>/)
