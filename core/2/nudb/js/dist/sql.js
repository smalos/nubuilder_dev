...
Sql.getFieldName = function ($tableResults, $thisField) {
  const thisFieldIndex = $thisField.index() // ltr or rtl direction does not impact how the DOM was generated
  // check if the action column in the left exist

  const leftActionExist = !$tableResults.find('th').first().hasClass('draggable') // number of column span for checkbox and Actions

  const leftActionSkip = leftActionExist ? $tableResults.find('th').first().attr('colspan') - 1 : 0 // If this column was sorted, the text of the a element contains something
  ...
  if (fieldName === '') {
    const $heading = $tableResults.find('thead').find('th').eq(thisFieldIndex - leftActionSkip).children('span') // may contain column comment enclosed in a span - detach it temporarily to read the column name
...
AJAX.registerOnload('sql.js', function () {
  ...
  $(function () {
    if (codeMirrorEditor) {
      ...
      const useLocalStorageValue = isStorageSupported('localStorage') && typeof window.localStorage.autoSavedSqlSort !== 'undefined' // Save sql query with sort
      ...
      const sortStoredQuery = useLocalStorageValue ? window.localStorage.autoSavedSqlSort : Cookies.get('autoSavedSqlSort')
  ...
  $(document).on('click', 'a.delete_row.ajax', function (e) {
    e.preventDefault()
    const question = Functions.sprintf(Messages.strDoYouReally, Functions.escapeHtml($(this).closest('td').find('div').text()))
    const $link = $(this)
  ...
  $(document).on('click', '#button_submit_query', function () {
    ...
    const $form = $(this).closest('form') // the Go button related to query submission was clicked,
  ...
  $(document).on('keyup', '.filter_rows', function () {
    ...
    const targetColumns = [] // To handle colspan=4, in case of edit,copy etc options.

    const dummyTh = $('.edit_row_anchor').length !== 0 ? '<th class="hide dummy_th"></th><th class="hide dummy_th"></th><th class="hide dummy_th"></th>' : '' // Selecting columns that will be considered for filtering and searching.

    $headerCells.each(function () {
      targetColumns.push($(this).text().trim())
    })
    const phrase = $(this).val() // Set same value to both Filter rows fields.
  ...
  $(document).on('submit', '.maxRowsForm', function () {
    const unlimNumRows = $(this).find('input[name="unlim_num_rows"]').val()
    const maxRowsCheck = Functions.checkFormElementInRange(this, 'session_max_rows', Messages.strNotValidRowNumber, 1)
    const posCheck = Functions.checkFormElementInRange(this, 'pos', Messages.strNotValidRowNumber, 0, unlimNumRows > 0 ? unlimNumRows - 1 : null)
...
Sql.changeClassForColumn = function ($thisTh, newClass, isAddClass) {
  ...
  const hasBigT = $thisTh.closest('tr').children().first().hasClass('column_action') // .eq() is zero-based
...
Sql.browseForeignDialog = function ($thisA) {
  ...
  $.post($thisA.attr('href'), params, function (data) {
    ...
    $(tableId).on('click', 'td a.foreign_value', function (e) {
      e.preventDefault()
      let $input = $thisA.prev('input[type=text]') // Check if input exists or get CEdit edit_box
    ...
    $(formId).on('submit', function (e) {
      ...
      const postParams = $(this).serializeArray() // if showAll button was clicked to submit form then
