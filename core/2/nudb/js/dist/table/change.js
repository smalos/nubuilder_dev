...
function nullify (theType, urlField, md5Field, multiEdit) {
  ...
  if (Number(theType) === 1) {
    ...
    const elts = rowForm.elements['fields' + multiEdit + '[' + md5Field + ']'] // when there is just one option in ENUM:
...
function verifyAfterSearchFieldChange (index, searchFormId) {
  const $thisInput = $('input[name=\'criteriaValues[' + index + ']\']') // Add  data-skip-validators attribute to skip validation in changeValueFieldType function
...
function validateIntField (jqueryInput, returnValueIfIsNumber) {
  ...
  const maxi = parseInt(jqueryInput.data('max')) // removing previous rules
...
function verificationsAfterFieldChange (urlField, multiEdit, theType) {
  ...
  const $thisInput = $(':input[name^=\'fields[multi_edit][' + multiEdit + '][' + urlField + ']\']') // the function drop-down that corresponds to this input field

  const $thisFunction = $('select[name=\'funcs[multi_edit][' + multiEdit + '][' + urlField + ']\']')
  ...
  const newSaltBox = '<br><input type=text name=salt[multi_edit][' + multiEdit + '][' + urlField + ']' + ' id=salt_' + target.id + ' placeholder=\'' + Messages.strEncryptionKey + '\'>' // If encrypting or decrypting functions that take salt as input is selected append the new textbox for salt
  ...
  if (target.value === 'HEX' && theType.substring(0, 3) === 'int') {
    ...
    const newHexInfo = '<br><p id="note' + target.id + '">' + Messages.HexConversionInfo + '</p>'
...
AJAX.registerOnload('table/change.js', function () {
  ...
  $(document).on('click', 'span.open_gis_editor', function (event) {
    ...
    const field = $span.parents('tr').children('td').first().find('input[type=\'hidden\']').val() // Column type

    const type = $span.parents('tr').find('span.column_type').text() // Names of input field and null checkbox

    const inputName = $span.parent('td').children('input[type=\'text\']').attr('name')
    openGISEditor()
  ...
  $(document).on('click', 'input[name=\'gis_data[save]\']', function () {
    ...
    const rowId = currentRow.find('.open_gis_editor').data('row-id') // Unchecks the Ignore checkbox for the current row
...
function addNewContinueInsertionFields (event) {
  ...
  const targetRows = $('#insert_rows').val() // remove all datepickers
  ...
  if (currRows < targetRows) {
    ...
    const tempReplaceAnchor = function () {
      const $anchor = $(this)
      const newValue = 'rownumber=' + newRowIndex // needs improvement in case something else inside
    ...
    while (currRows < targetRows) {
       ...
       */
      const $lastRow = $('#insertForm').find('.insertRowTable').last() // need to access this at more than one level
      ...
      if (currRows === 1) {
        ...
        const newName = lastCheckboxName.replace(/\d+/, lastCheckboxIndex + 1)
...
function changeValueFieldType (elem, searchIndex) {
  ...
  if (type === 'LIKE' || type === 'LIKE %...%' || type === 'NOT LIKE' || type === 'NOT LIKE %...%') {
  ...
  if (type === 'IN (...)' || type === 'NOT IN (...)' || type === 'BETWEEN' || type === 'NOT BETWEEN') {
