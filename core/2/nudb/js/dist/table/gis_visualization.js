...
let defaultX = 0
const defaultY = 0
const defaultScale = 1 // Variables
...
function zoomAndPan () {
  const g = gisSvg.getElementById('groupPanel')
...
function addZoomPanControllers () {
  ...
  const themeImagePath = $('#themeImagePath').val()
...
function resizeGISVisualization () {
  const $placeholder = $('#placeholder')
  const oldWidth = $placeholder.width()
  const visWidth = $('#div_view_options').width() - 48 // Assign new value for width
...
function getRelativeCoords (e) {
  const position = $('#placeholder').offset()
  return {
...
function onGisMouseWheel (event) {
  ...
  const relCoords = getRelativeCoords(event)
  const factor = event.deltaY > 0 ? zoomFactor : 1 / zoomFactor // zoom
...
AJAX.registerOnload('table/gis_visualization.js', function () {
  ...
  $(document).on('drag', 'svg', function (event, dd) {
    const newX = Math.round(dd.offset.left)
    x += newX - dragX
    dragX = newX
    const newY = Math.round(dd.offset.top)
    y += newY - dragY
    dragY = newY
    zoomAndPan()
  })
  ...
  $(document).on('click', '#zoom_in', function (e) {
    ...
    scale *= zoomFactor
    const width = $(gisSvg).attr('width')
    const height = $(gisSvg).attr('height') // zooming in keeping the center unmoved.
  ...
  $(document).on('click', '#zoom_out', function (e) {
    ...
    scale /= zoomFactor
    const width = $(gisSvg).attr('width')
    const height = $(gisSvg).attr('height') // zooming out keeping the center unmoved.
  ...
  $('.vector').on('mousemove', function (event) {
    const contents = Functions.escapeHtml($(this).attr('name')).trim()
    $('#tooltip').remove()
