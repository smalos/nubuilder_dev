...
TableRelation.setDropdownValues = function ($dropdown, values, selectedValue) {
  $dropdown.empty()
  let optionsAsString = '' // add an empty string to the beginning for empty selection
...
TableRelation.getDropdownValues = function ($dropdown) {
  ...
  let $tableDd
  let $columnDd
  let foreign = '' // if the changed dropdown is for foreign key constraints
    ...
    success: function (data) {
      ...
      if (typeof data !== 'undefined' && data.success) {
        ...
        if (foreignTable === null) {
          ...
          if (typeof data.primary !== 'undefined' && data.primary.length === 1) {
...
AJAX.registerOnload('table/relation.js', function () {
  ...
  $('body').on('click', 'a.add_foreign_key_field', function (event) {
    ...
    const $sourceElem = $('select[name^="destination_foreign_column[' + $(this).attr('data-index') + ']"]').last().parent()
  ...
  $('body').on('click', 'a.add_foreign_key', function (event) {
    ...
    const $newRow = $prevRow.clone(true, true) // Update serial number.
  ...
  $('a.drop_foreign_key_anchor.ajax').on('click', function (event) {
    event.preventDefault()
    const $anchor = $(this) // Object containing reference to the current field's row

    const $currRow = $anchor.parents('tr')
    const dropQuery = Functions.escapeHtml($currRow.children('td').children('.drop_foreign_key_msg').val())
