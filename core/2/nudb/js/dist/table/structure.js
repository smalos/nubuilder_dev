...
AJAX.registerOnload('table/structure.js', function () {
  ...
  $(document).on('submit', '.append_fields_form.ajax', function (event) {
    ...
    const $form = $(this)
    const fieldCnt = $form.find('input[name=orig_num_fields]').val()

    function submitForm () {
      const $msg = Functions.ajaxShowMessage(Messages.strProcessingRequest)
    ...
    if (Functions.checkTableEditForm($form[0], fieldCnt)) {
      ...
      if (Functions.checkReservedWordColumns($form)) {
        ...
        if (checkIfConfirmRequired($form)) {
          const question = sprintf(Messages.strChangeColumnCollation, 'https://wiki.phpmyadmin.net/pma/Garbled_data')
  ...
  $(document).on('click', 'a.drop_column_anchor.ajax', function (event) {
    ...
    const question = Functions.sprintf(Messages.strDoYouReally, 'ALTER TABLE `' + currTableName + '` DROP `' + currColumnName + '`;')
    const $thisAnchor = $(this)
    $thisAnchor.confirm(question, $thisAnchor.attr('href'), function (url) {
      const $msg = Functions.ajaxShowMessage(Messages.strDroppingColumn, false)
      let params = Functions.getJsConfirmCommonParam(this, $thisAnchor.getPostData())
  ...
  $(document).on('click', 'a.add_key.ajax', function (event) {
    ...
    const question = Functions.sprintf(Messages.strDoYouReally, 'ALTER TABLE `' + Functions.escapeHtml(currTableName) + '` ' + addClause + '(`' + Functions.escapeHtml(currColumnName) + '`);')
    const $thisAnchor = $(this)
    $thisAnchor.confirm(question, $thisAnchor.attr('href'), function (url) {
  ...
  **/
  ...
  $(document).on('click', '#move_columns_anchor', function (e) {
    ...
    $('#tablestructure').find('tbody tr').each(function () {
      const colName = $(this).find('input:checkbox').eq(0).val()
      const hiddenInput = $('<input>').prop({
        name: 'move_columns[]',
        type: 'hidden'
      }).val(colName)
      columns[columns.length] = $('<li></li>').addClass('placeholderDrag').text(colName).append(hiddenInput)
    })
    const colList = $('#move_columns_dialog').find('ul').find('li').remove().end()
    ...
    $('#designerModalGoButton').on('click', function () {
      ...
      const serialized = $form.serialize() // check if any columns were moved at all
      ...
      $.post($form.prop('action'), serialized + CommonParams.get('arg_separator') + 'ajax_request=true', function (data) {
        if (data.success === false) {
          ...
          const $fieldsTable = $('table#tablestructure tbody') // remove all existing rows and remember them

          const $rows = $fieldsTable.find('tr').remove() // loop through the correct order

          for (const i in data.columns) {
            const theColumn = data.columns[i]
            const $theRow = $rows.find('input:checkbox[value=\'' + theColumn + '\']').closest('tr') // append the row for this column to the table

            $fieldsTable.append($theRow)
          }

          const $firstrow = $fieldsTable.find('tr').eq(0) // Adjust the row numbers and colors
  ...
  $(document).on('click', '#remove_partitioning.ajax', function (e) {
    e.preventDefault()
    const $link = $(this)
    const question = Messages.strRemovePartitioningWarning
    $link.confirm(question, $link.attr('href'), function (url) {
      const params = Functions.getJsConfirmCommonParam({
        ajax_request: true,
