...
AJAX.registerOnload('database/qbe.js', function () {
  ...
  $('#tblQbe').on('resize', function () {
    const newWidthTblQbe = $('#textSqlquery').next().width()
  ...
  $(document).on('change', 'select[name^=criteriaColumn]', function () {
    if ($(this).val()) {
      const index = (/\d+/).exec($(this).attr('name'))
  ...
  $(document).on('click', '#deleteSearch', function () {
    const question = Functions.sprintf(Messages.strConfirmDeleteQBESearch, $('#searchId').find('option:selected').text())
    if (!confirm(question)) {
      return false
    }
