...
DesignerMove.mouseMove = function (e) {
  ...
  // eslint-disable-next-line compat/compat
  const newDx = isIe ? e.clientX + document.body.scrollLeft : e.pageX
  // eslint-disable-next-line compat/compat
    ...
    for (key in contr[K]) {
      ...
      for (key2 in contr[K][key]) {
        ...
        for (key3 in contr[K][key][key2]) {
          ...
          if (tabHideButton.innerHTML === 'v') {
            const toColumn = document.getElementById(contr[K][key][key2][key3][0] +
...
DesignerMove.promptToSaveCurrentPage = function (callback) {
  if (change === 1 || selectedPage === -1) {
    const modal = DesignerMove.displayModal('<div>' + Messages.strLeavingPage + '</div>',
...
DesignerMove.clickField = function (db, T, f, pk) {
  ...
  if (onDisplayField) {
    ...
    const oldTabField = document.getElementById('id_tr_' + T + '.' + displayField[T])
    // if is display field
...
DesignerMove.canvasClick = function (id, event) {
  ...
  let key3
  // eslint-disable-next-line compat/compat
  let localX = isIe ? event.clientX + document.body.scrollLeft : event.pageX
  // eslint-disable-next-line compat/compat
    ...
    for (key in contr[K]) {
      for (key2 in contr[K][key]) {
        for (key3 in contr[K][key][key2]) {
          ...
          const y2 = document.getElementById(contr[K][key][key2][key3][0]).offsetTop +
