...
Functions.confirmQuery = function (theForm1, sqlQuery1) {
  ...
  if (doConfirmRegExp0.test(sqlQuery1) ||
    ...
    const isConfirmed = confirm(Functions.sprintf(Messages.strDoYouReally, message))
...
Functions.sqlPrettyPrint = function (string) {
  ...
  const spaceExceptionsBefore = { ';': true, ',': true, '.': true, '(': true }
...
AJAX.registerOnload('functions.js', function () {
  ...
  $(document).on('click', 'a.central_columns_dialog', function () {
    ...
    const centralColumnsDialog = '<div class=\'max_height_400\'>' +
            '<fieldset class="pma-fieldset">' +
            searchIn +
