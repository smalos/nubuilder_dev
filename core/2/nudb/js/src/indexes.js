...
Indexes.removeColumnFromIndex = function (colIndex) {
  ...
  let previousIndex = $('select[name="field_key[' + colIndex + ']"]')
    .attr('data-index')
...
Indexes.indexTypeSelectionDialog = function (sourceArray, indexChoice, colIndex) {
  const $singleColumnRadio = $('<input type="radio" id="single_column" name="index_choice"' +
        ' checked="checked">' +
        '<label for="single_column">' + Messages.strCreateSingleColumnIndex + '</label>')
  const $compositeIndexRadio = $('<input type="radio" id="composite_index"' +
        ' name="index_choice">' +
...
AJAX.registerOnload('indexes.js', function () {
  ...
  $(document).on('click', '#table_index tbody tr td.edit_index.ajax, #index_div .add_index.ajax', function (event) {
    ...
    if ($(this).find('a').length === 0) {
      // Add index
      const valid = Functions.checkFormElementInRange(
        $(this).closest('form')[0],
        'added_fields',
