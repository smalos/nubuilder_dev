...
function goTo3NFFinish (newTables) {
  for (const table in newTables) {
    for (const newtbl in newTables[table]) {
      const updatedname = $('#extra input[name="' + newtbl + '"]').val()
...
function goTo3NFStep2 (pd, tablesTds) {
  ...
  let extra = '<div class="dependencies_box">'
  let pdFound = false
  for (const table in tablesTds) {
    for (const i in tablesTds[table]) {
...
AJAX.registerOnload('normalization.js', function () {
  ...
  $('#extra').on('click', '#moveRepeatingGroup', function () {
    ...
    if (repeatingCols !== '') {
      const newColName = $('#extra input[type=checkbox]:checked').first().val()
      repeatingCols = repeatingCols.slice(0, -2)
      let confirmStr = Functions.sprintf(Messages.strMoveRepeatingGroup, Functions.escapeHtml(repeatingCols), Functions.escapeHtml(CommonParams.get('table')))
  ...
  $('#mainContent p').on('click', '#createPrimaryKey', function (event) {
    ...
    const title = Messages.strAddPrimaryKey
