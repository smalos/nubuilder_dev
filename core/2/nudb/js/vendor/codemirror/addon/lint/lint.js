...
(function (mod) {
  ...
  function makeMarker (cm, labels, severity, multiple, tooltips) {
    const marker = document.createElement('div'); let inner = marker
  ...
  function groupByLine (annotations) {
    ...
    for (let i = 0; i < annotations.length; ++i) {
      const ann = annotations[i]; const line = ann.from.line;
  ...
  function onMouseOver (cm, e) {
    ...
    const box = target.getBoundingClientRect(); const x = (box.left + box.right) / 2; const y = (box.top + box.bottom) / 2
  ...
  CodeMirror.defineOption('lint', false, function (cm, val, old) {
    ...
    if (val) {
      const gutters = cm.getOption('gutters'); let hasLintGutter = false
