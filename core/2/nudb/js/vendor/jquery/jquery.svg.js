...
/** The SVG manager.
  ...
  $.extend(SVGManager.prototype, {
    ...
    _afterLoad: function (container, svg, settings) {
      ...
      const wrapper = new this._wrapperClass(svg, container)
  ...
  function SVGWrapper (svg, container) {
    ...
    for (let i = 0; i < $.svg._extensions.length; i++) {
      const extension = $.svg._extensions[i]
  ...
  $.extend(SVGWrapper.prototype, {
    ...
    title: function (parent, text, settings) {
      const args = this._args(arguments, ['text'])
      const node = this._makeNode(args.parent, 'title', args.settings || {})
    ...
    describe: function (parent, text, settings) {
      const args = this._args(arguments, ['text'])
      const node = this._makeNode(args.parent, 'desc', args.settings || {})
      node.appendChild(this._svg.ownerDocument.createTextNode(args.text))
      return node
    },
    ...
    defs: function (parent, id, settings) {
      const args = this._args(arguments, ['id'], ['id'])
    ...
    symbol: function (parent, id, x1, y1, width, height, settings) {
      const args = this._args(arguments, ['id', 'x1', 'y1', 'width', 'height'])
    ...
    marker: function (parent, id, refX, refY, mWidth, mHeight, orient, settings) {
      const args = this._args(arguments, ['id', 'refX', 'refY', 'mWidth', 'mHeight', 'orient'], ['orient'])
    ...
    style: function (parent, styles, settings) {
      const args = this._args(arguments, ['styles'])
      const node = this._makeNode(args.parent, 'style', $.extend({ type: 'text/css' }, args.settings || {}))
    ...
    script: function (parent, script, type, settings) {
      const args = this._args(arguments, ['script', 'type'], ['type'])
      const node = this._makeNode(args.parent, 'script', $.extend(
    ...
    linearGradient: function (parent, id, stops, x1, y1, x2, y2, settings) {
      const args = this._args(arguments, ['id', 'stops', 'x1', 'y1', 'x2', 'y2'], ['x1'])
      const sets = $.extend({ id: args.id },
    ...
    radialGradient: function (parent, id, stops, cx, cy, r, fx, fy, settings) {
      const args = this._args(arguments, ['id', 'stops', 'cx', 'cy', 'r', 'fx', 'fy'], ['cx'])
      const sets = $.extend({ id: args.id },
    ...
    _gradient: function (parent, name, settings, stops) {
      const node = this._makeNode(parent, name, settings)
    ...
    pattern: function (parent, id, x, y, width, height, vx, vy, vwidth, vheight, settings) {
      const args = this._args(arguments, ['id', 'x', 'y', 'width', 'height', 'vx', 'vy', 'vwidth', 'vheight'], ['vx'])
      const sets = $.extend({ id: args.id, x: args.x, y: args.y, width: args.width, height: args.height },
    ...
    clipPath: function (parent, id, units, settings) {
      const args = this._args(arguments, ['id', 'units'])
    ...
    mask: function (parent, id, x, y, width, height, settings) {
      const args = this._args(arguments, ['id', 'x', 'y', 'width', 'height'])
    ...
    svg: function (parent, x, y, width, height, vx, vy, vwidth, vheight, settings) {
      const args = this._args(arguments, ['x', 'y', 'width', 'height', 'vx', 'vy', 'vwidth', 'vheight'], ['vx'])
      const sets = $.extend({ x: args.x, y: args.y, width: args.width, height: args.height },
        (args.vx != null ? { viewBox: args.vx + ' ' + args.vy + ' ' + args.vwidth + ' ' + args.vheight } : {}))
    ...
    use: function (parent, x, y, width, height, ref, settings) {
      const args = this._args(arguments, ['x', 'y', 'width', 'height', 'ref'])
      if (typeof args.x === 'string') {
        args.ref = args.x
        args.settings = args.y
        args.x = args.y = args.width = args.height = null
      }
      const node = this._makeNode(args.parent, 'use', $.extend(
    ...
    link: function (parent, ref, settings) {
      const args = this._args(arguments, ['ref'])
      const node = this._makeNode(args.parent, 'a', args.settings)
      node.setAttributeNS($.svg.xlinkNS, 'href', args.ref)
      return node
    },
    ...
    image: function (parent, x, y, width, height, ref, settings) {
      const args = this._args(arguments, ['x', 'y', 'width', 'height', 'ref'])
      const node = this._makeNode(args.parent, 'image', $.extend(
        { x: args.x, y: args.y, width: args.width, height: args.height }, args.settings || {}))
      node.setAttributeNS($.svg.xlinkNS, 'href', args.ref)
      return node
    },
    ...
    path: function (parent, path, settings) {
      const args = this._args(arguments, ['path'])
    ...
    rect: function (parent, x, y, width, height, rx, ry, settings) {
      const args = this._args(arguments, ['x', 'y', 'width', 'height', 'rx', 'ry'], ['rx'])
    ...
    circle: function (parent, cx, cy, r, settings) {
      const args = this._args(arguments, ['cx', 'cy', 'r'])
    ...
    ellipse: function (parent, cx, cy, rx, ry, settings) {
      const args = this._args(arguments, ['cx', 'cy', 'rx', 'ry'])
    ...
    line: function (parent, x1, y1, x2, y2, settings) {
      const args = this._args(arguments, ['x1', 'y1', 'x2', 'y2'])
    ...
    polyline: function (parent, points, settings) {
      const args = this._args(arguments, ['points'])
    ...
    text: function (parent, x, y, value, settings) {
      const args = this._args(arguments, ['x', 'y', 'value'])
    ...
    textpath: function (parent, path, value, settings) {
      const args = this._args(arguments, ['path', 'value'])
      const node = this._text(args.parent, 'textPath', args.value, args.settings || {})
    ...
    other: function (parent, name, settings) {
      const args = this._args(arguments, ['name'])
    ...
    _makeNode: function (parent, name, settings) {
      ...
      const node = this._svg.ownerDocument.createElementNS($.svg.svgNS, name)
    ...
    add: function (parent, node) {
      const args = this._args((arguments.length === 1 ? [null, parent] : arguments), ['node'])
      const svg = this
      ...
      try {
        ...
        args.node.each(function () {
          const child = svg._cloneAsSVG(this)
          if (child) {
    ...
    _cloneAsSVG: function (node) {
      ...
      if (node.nodeType === 1) { // element
        ...
        for (var i = 0; i < node.childNodes.length; i++) {
          const child = this._cloneAsSVG(node.childNodes[i])
          if (child) {
    ...
    load: function (url, settings) {
      ...
      const size = [this._svg.getAttribute('width'), this._svg.getAttribute('height')]
      const wrapper = this
      ...
      const loadXML4IE = function (data) {
      ...
      const loadSVG = function (data) {
        ...
        if (data.documentElement.nodeName !== 'svg') {
          const errors = data.getElementsByTagName('parsererror')
          const messages = (errors.length ? errors[0].getElementsByTagName('div') : []) // Safari
          reportError(!errors.length ? '???' : (messages.length ? messages[0] : errors[0]).firstChild.nodeValue)
          return
        }
        const parent = (settings.parent ? $(settings.parent)[0] : wrapper._svg)
        const attrs = {}
        for (var i = 0; i < data.documentElement.attributes.length; i++) {
          const attr = data.documentElement.attributes.item(i)
          if (!(attr.nodeName === 'version' || attr.nodeName.substring(0, 5) === 'xmlns')) {
            attrs[attr.nodeName] = attr.nodeValue
          }
        }
        wrapper.configure(parent, attrs, !settings.parent)
        const nodes = data.documentElement.childNodes
