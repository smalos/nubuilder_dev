...
$.fn.nuFocusWithoutScrolling = function () {
  const t = $(this)[0]
  const x = t.scrollX; const y = t.scrollY
...
function nuDisplayError (e) {
  ...
  const msgDiv = nuMessage(e.errors)
...
function nuLogin (loginTopRow, nuconfigNuWelcomeBodyInnerHTML, logonMode = 'normal', onlySsoExcept = {}, lastUser = '') {
  ...
  const topRow = loginTopRow == '' ? defaulTopRow : loginTopRow
...
function nuBuildLookup (t, s, like) {
  ...
  const tar = obj.attr('data-nu-target')
  const p = obj.attr('data-nu-prefix')
...
function nuCreateDialog (t) {
  ...
  this.moveDialog = function (e) {
    ...
    const o = s.style
    const l = parseInt(o.left, 10) + this.moveX
    const t = parseInt(o.top, 10) + this.moveY
...
function nuReformat (t) {
  function nuReapplyFormat (v, f) {
    const r = nuFORM.removeFormatting(v, f)
...
function nuBindCtrlEvents () {
  ...
  $(document).keydown(function (e) {
    ...
    if ((nuIsMacintosh() ? e.metaKey : e.ctrlKey) && e.shiftKey) {
      ...
      const g = nuGlobalAccess()
      const formId = window.nuFORM.getCurrent().form_id
...
function nuTranslateToEnglish (str) {
  ...
  const l = nuLANGUAGE.find(elem => elem.translation === str)
...
function nuResizeWindow (e) {
  ...
  if (l == 2) {
    ...
    const dh = parseInt(d.css('height'), 10) - 50
    const dw = parseInt(d.css('width'), 10) - W - 10
...
function nuAttachFontAwesome (i, c, s, after) {
  ...
  const h = obj.html().trim()
  const nbsp = h.length == 0 ? '' : '&nbsp'
...
function nuCurrentDate (format) {
  ...
  const
    yyyy = d.getFullYear()
  const mm = nuPad2(d.getMonth() + 1)
  const dd = nuPad2(d.getDate())
...
function nuCurrentDateTime (format) {
  ...
  const mm = nuPad2(d.getMonth() + 1)
  const dd = nuPad2(d.getDate())
  const hh = nuPad2(d.getHours())
  const nn = nuPad2(d.getMinutes())
  const ss = nuPad2(d.getSeconds())
...
function nuBase64encode (str) {
  const encode = encodeURIComponent(str).replace(/%([a-f0-9]{2})/gi, (m, $1) => String.fromCharCode(parseInt($1, 10)))
  return btoa(encode)
}
...
function nuSetLabelText (i, str, translate) {
  ...
  const lwidth = nuGetWordWidth(str)

  const obj = $('#' + i)
  const left = obj.cssNumber('left')
  const top = obj.cssNumber('top')
...
function nuAddCSSStyle (styleString, id) {
  const i = id === undefined ? 'nucssstyle' : id
  $('#' + i).remove()
