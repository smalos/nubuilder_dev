...
function nuCreateDrag () {
  ...
  this.setSelected = function () {
    ...
    for (let i = 0; i < nuREPORT.selected.length; i++) {
  ...
  this.move = function (event) {
    ...
    const key = event.key
    const m = (event.buttons == 1 && this.classList.indexOf('nuDragSelected') != -1)
  ...
  this.multipleSections = function () {
    const s = $('.nuDragSelected').attr('data-order')
    const a = $('.nuDragSelected')
    let b = false
  ...
  this.resizeBox = function (event) {
    ...
    const Y = event.clientY + window.scrollY - this.startY
  ...
  this.removeBox = function () {
    ...
    const L = parseInt($sb.css('left'), 10) - this.areaLeft()
    const T = parseInt($sb.css('top'), 10) - this.areaTop()
    const B = T + parseInt($sb.css('height'), 10)
    const R = L + parseInt($sb.css('width'), 10)

    $sb.remove()

    const o = $('.nuDragObject')
