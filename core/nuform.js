...
function nuSubformPaste (e, jsonObj) {
  ...
  const field = $('#' + id).attr('data-nu-field')
  const dRow = parseInt($('#' + String(id), 10).attr('data-nu-prefix').slice(-3))
...
function nuObjectPosition (i) {
  const t = parseInt($(i).css('top'), 10)
  const l = parseInt($(i).css('left'), 10)
  const h = parseInt($(i).css('height'), 10)
  const w = parseInt($(i).css('width'), 10)
