...
function nuAddHome () {
  if (window.nuLoginH != '') {
    const breadCrumb = $('#nuBreadcrumb0').length > 0 ? $('#nuBreadcrumb0') : $('#nuHomeGap')
    breadCrumb
...
function nuNeedToLoginAgain (f) {
  const result = f.tableSchema === null
  if (result) {
...
function nuAddActionButton (id, value, func, text, e) {
  ...
  id = 'nu' + id + 'Button'
  const html = `<button id='${id}' type='button' class='${nuClass}' title = '${text}' onclick='${func}'>${value}` + '</button>'
...
function nuLabel (w, i, p, prop) {
  ...
  if (nuGlobalAccess()) { $('#' + id).attr('ondblclick', 'nuOptionsListAction("nuobject", "' + obj.object_id + '")') }
...
function nuSavingProgressMessage () {
  const div = nuCreateElementWithId('div', 'nuProgressUpdate', 'nuActionHolder')
  $div = $(div)
...
function nuUpdateMessage (actionMessage) {
  ...
  const div = nuCreateElementWithId('div', 'nuNowUpdated', 'nuActionHolder')
  $div = $(div)
...
function nuAddBrowseAdditionalNavButtons () {
  if (nuFormType() == 'browse') {
    ...
    let html = '<span id="nuFirst" class="nuBrowsePage" style="font-size: 15px;"><i class="fa fa-step-backward ml-5 mr-5" onclick="nuGetPage(1)">&nbsp;&nbsp;&nbsp;&nbsp;</i></span>'
