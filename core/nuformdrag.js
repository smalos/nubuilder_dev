...
function nuSetTabOrderDataAttrs () {
  const currentTabNo	= $('div.nuTabSelected[id^="nuTab"]').attr('data-nu-tab-filter') || '0'
...
function nuMoveUpOrder () {
  ...
  for (let i = 0; i < getNuDragDialogIframes()[0].contentWindow.nuDragOptionsState.tabs[currentTabNo].objects.length; i++) {
    const field							= getNuDragDialogIframes()[0].contentWindow.nuDragOptionsState.tabs[currentTabNo].objects[i]
...
function nuFindFieldInState (tabNo, fieldID) {
  for (let i = 0; i < getNuDragDialogIframes()[0].contentWindow.nuDragOptionsState.tabs[tabNo].objects.length; i++) {
...
function nuSpacingNotSupported () {
  const supported = $('.nuDragSelected', getNuDragDialogIframes(true)).filter('.nu_contentbox').length == 0
  if (!supported) {
...
function nuPutFieldDimensionsIntoState () {
  for (let tabNo = 0; tabNo < getNuDragDialogIframes()[0].contentWindow.nuDragOptionsState.tabs.length; tabNo++) {
    for (let fieldNo = 0; fieldNo < getNuDragDialogIframes()[0].contentWindow.nuDragOptionsState.tabs[tabNo].objects.length; fieldNo++) {
      const field = getNuDragDialogIframes()[0].contentWindow.nuDragOptionsState.tabs[tabNo].objects[fieldNo]

      const contents = $('#' + field.id, getNuDragDialogIframes(true))
      const cb = $('div#frame_' + field.id, getNuDragDialogIframes(true))
...
function nuGetTopArea () {
  ...
  const dialogTitle			= parseInt(parent.window.$('#dialogTitle').css('height'), 10)
