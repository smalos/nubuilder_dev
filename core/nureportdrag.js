...
function nuLoadReport (b) {
  ...
  if (arguments.length == 0) {
    const r = JSON.stringify(nuREPORT)
...
function nuSetnuScroll (l) {
  const flds = ['#fieldName', "[id^='sortField']"]
...
function nuDragReport () {
  ...
  this.getObject = function (id) {
    for (let g = 0; g < nuREPORT.groups.length; g++) {
      for (let s = 0; s < nuREPORT.groups[g].sections.length; s++) {
        for (let o = 0; o < nuREPORT.groups[g].sections[s].objects.length; o++) {
  ...
  this.createSection = function (g, s) {
    const i = this.groups[g][s]
    const S = nuREPORT.groups[g].sections[s]

    const v = S.height == 0 ? 'hidden' : 'visible'
    const b = S.height == 0 ? 'none' : 'none none solid none'
    let t = opener.nuTranslate('Detail')
  ...
  this.createObject = function (t, o, i) {
    const w = { B: 'bold', N: 'normal', I: 'italic' }
    const id = o.id
    const e = document.createElement('div')
  ...
  this.adjustObjectsForMovedSection = function (event) {
    const m = event.clientY - this.moveFrom
    const g = $(this.beingMoved).attr('data-group')
    const s = $(this.beingMoved).attr('data-section')
  ...
  this.adjustSectionForMovedObjects = function (event) {
    ...
    const m = nuDrag.multipleSections()
    let min = 30
    let grp = 0
    let sec = 0
  ...
  this.resetObjectProperties = function (p, v) {
    ...
    for (let i = 0; i < sel.length; i++) {
      const o = this.getObject(sel[i].id)
      o[p] = v
      this.setObject(o)
    }
  ...
  this.resizeSection = function (g, s, c) {
    const i = this.groups[g][s]
    const l = this.lowestSectionObject(g, s)
    const h = nuRemovePX($('#nuSection' + i).css('height'))
    const m = Math.max(c, l - h)
  ...
  this.lowestSectionObject = function (g, s) {
    const i = this.groups[g][s]
    const T = nuRemovePX($('#nuSection' + i).css('top'))
    const a = $("[data-order='" + i + "']").filter('.nuDragObject')
    let t = 0
    let h = 0
    let b = 0
  ...
  this.move = function (event) {
    if ($('#nuDragLine').length == 1) {
      const s = document.getElementById('nuDragLine')
      const o = s.style
  ...
  this.down = function (event) {
    const lc = window.nuLastClick[event.target.id]
    ...
    if (String(event.target.id).substr(0, 10) == 'nuLeftMove') {
      ...
      this.startY = event.clientY
      const e = document.createElement('div')
      const t = $(this.beingMoved).offset().top
...
function nuNewObject () {
  nuREPORT.selected = []
  const o = JSON.parse(JSON.stringify(nuOBJECT))
  const i = nuDragR.newId()
  const l = nuREPORT.groups[0].sections[0].objects.length
...
function nuCloneObjects () {
  ...
  for (let i = 0; i < sel.length; i++) {
    const o = nuDragR.getObject(sel[i].id)
    const gs = nuDragR.getGroupAndSection(sel[i].id)
    const g = gs[0]
    const s = gs[1]
    const l = nuREPORT.groups[g].sections[s].objects.length
    const d = nuDragR.newId()
...
function nuSelectDialog () {
  ...
  const dragObj = document.getElementsByClassName('nuDragObject')
...
function nuAdjustButton (top, left, funct, value, title) {
  const e = document.createElement('input')			// -- create button
...
function nuMoverAlignTopClick () {
  ...
  let t = 100000
  nuDragR.dragging = true
  const gs = nuDragR.getGroupAndSection(s[0].id)
  ...
  for (var i = 0; i < s.length; i++) {
    ...
    const o = nuDragR.getObject(s[i].id)
    t = Math.min(t, o.top)
  }
...
function nuMoverAdjustVerClick () {
  ...
  const a = []
  let o = {}
  const d = document.getElementsByClassName('nuDragSelected')
  nuDragR.dragging = true
  ...
  const s = a.sort(function (A, B) { return A.top - B.top })
  const newGap = (b - t - h) / (a.length - 1)
  let newTop = t
...
function nuMoverAdjustHorClick () {
  ...
  const s = a.sort(function (A, B) { return A.left - B.left })
  const newGap = (b - l - w) / (a.length - 1)
  let newLeft = l
...
function nuObjectDialog () {
  ...
  const D = nuDragR.getObject(S[0].id)
  let top = 60
  const fun = 'nuUpdateProperties'
  ...
  let t = opener.nuTranslate('Field Name')
...
function nuBlankMultipleValues (S) {
  const f = nuDragR.getObject(S[0].id) // -- first Object
  const k = Object.keys(f)

  for (let i = 0; i < k.length; i++) {
    for (let j = 1; j < S.length; j++) {
      const o = nuDragR.getObject(S[j].id)
...
function nuUpdateGroup (t) {
  ...
  const group = nuREPORT.groups[dataGroup]
...
function nuUpdateProperties (id) {
  ...
  for (let i = 0; i < sel.length; i++) {
    const obj = nuDragR.getObject(sel[i].id)
...
function nuClickGroup (id) {
  const group = $(id).attr('data-group')
...
function nuUpdateSectionProperty (t) {
  const $t = $(t)
  const g = nuDIALOG.groupNumber
  const s = $t.attr('data-section')
  const p = $t.attr('data-property')
  let v = $t.val()
