...
class nuSECTION
{
    ...
    private function setObjectLines($O, $stopGrow = false)
    {
        ...
        for ($i = 0 ; $i < $countO ; $i++) {
            ...
            if ($O[$i]->objectType == 'field' or $O[$i]->objectType == 'label') {
            ...
            } elseif ($O[$i]->objectType == 'image') {									//-- image
                $O[$i]->LINES				= [''];
                $path						= '';
                $fld						= $O[$i]->fieldName;
                ...
                if (nuIsField($fld)) {													//-- FIELD NAME
                    ...
                    if ($j != '') {
                        $path				= nuCreateFile($j);
                        $GLOBALS['nu_files'][] = $path;
                    } elseif (nuIsFile($fld)) {											//-- FILE NAME on hard drive
                        $path				= $fld;
                    }
                }
...
function nuPrintField($PDF, $S, $contents, $O, $LAY, $isHTML)
{
    ...
    if (isset($contents->B)) {
        $backgroundColor = $contents->B;
    }
    if (isset($contents->F)) {
        $fontColor = $contents->F;
    }
...
function nuIsFile($i)
{
    ...
    if ($file_headers[0] == 'HTTP/1.0 404 Not Found') {
        return false;
    } elseif ($file_headers[0] == 'HTTP/1.0 302 Found') {
        return false;
    } else {
        return true;
    }
