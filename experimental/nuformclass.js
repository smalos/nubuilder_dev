...
class nuFormObject {
  ...
  selectFields () {									// -- from SELECT builder
    ...
    sqlFrameContents.find('.nuBox').each(function (index) {
      const b = $(this)[0].id
      let a = sqlFrameContents.find('#alias' + b).val()
      const t = sqlFrameContents.find('#tablename' + b).html()
  ...
  subform (sf, action = 'save') {
    ...
    $(sel).each(function (index) {
      ...
      THIS.children('[data-nu-data]').each(function () {
        const children = cThis.subformCollectRecordChildren(this, sf, chk, deleteAll, V, E, F, C)
        chk = children.chk
        V = children.V
        E = children.E
        C = children.C
      ...
      THIS.children().children('[data-nu-data]').each(function () {
        const children = cThis.subformCollectRecordChildren(this, sf, chk, deleteAll, V, E, F, C)
        chk = children.chk
        V = children.V
        E = children.E
  ...
  addFormatting (v, f) {
    ...
    if (f[0] == 'D') {	// -- date
      ...
      const wee = splitDate[0]													// -- Tue Jul 07 2022 11:11:12 GMT+0930 (Cen. Australia Standard Time)
      const mth = splitDate[1]
      const day = splitDate[2]
      const yea = splitDate[3]

      const hou = String(splitDate[4]).split(':')[0]
      const min = String(splitDate[4]).split(':')[1]
      const sec = String(splitDate[4]).split(':')[2]
