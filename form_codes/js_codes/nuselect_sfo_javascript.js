...
if (window.filter == 'justsql') {
  const sid = String(nuFORM.getCurrent().record_id)
  const from = sid.substring(sid.length - 2)
  const targ = '#sfo_browse_sql'
...
function nuWhereClauses () {
  $("[id$='ssc_type']select").each(function (index) {
    const p = $(this).attr('data-nu-prefix')
    const t = $(this).val()
...
function nuAddSQLTable (e) {
  const s = $('#sqlframe')[0].contentWindow.nuSQL
