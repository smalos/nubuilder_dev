...
function inDBVersion () {
  const dbInc = incVersion('set_db_version')
  $('#set_db_version_inc').val(dbInc)
}

function inFilesVersion () {
  const filesInc = incVersion('set_files_version')
...
function nuBeforeSave () {
  ...
  if (v === '') {
    const languagesIncluded = selectToValueArray('set_languages_included')
...
function onConfigValueChanged (t) {
  const orgValue = nuBase64decode($(t).attr('data-org-value'))
  if (orgValue !== t.value) {
...
function sfSetDescriptionTitle () {
  const fieldArr = nuSubformColumnArray('nuconfigsettings', 'cfg_description')

  for (let i = 0; i < fieldArr.length; i++) {
    const f = $('#nuconfigsettings' + nuPad3(i) + 'cfg_description')
    if (f.val() !== '') {
      f.attr('title', f.val())
    }
  }
...
function selectAddBooleanOptions (el, val) {
  ...
  const index = val == 'true' ? 0 : (val == 'false' ? 1 : -1)
...
function sfChangeValueObjectTypes () {
  ...
  for (let i = 0; i < sf.rows.length; i++) {
    ...
    if (type == '2') {
      const selectObj = changeObjectTypeToSelectBoolean(el)
